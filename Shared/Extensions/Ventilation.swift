        ...
        if PL >= 0 {
            ...
            if RRATE > 1 {
                if COADJ > 0.5 {
                    U = AO2PR * 0.15
                    if U > 70 {U = 70}
                    DSPAC = dampChange((c52+DVENT*100.0/pow(RRATE,1.12)+20.0*DVENT/(COADJ+5.0)+U+FY+c54*(TIDVL+500.0)), oldValue: DSPAC, dampConstant: c55)
                }
            }
...
//            1400 FVENT=AVENT*c56
            ...
            if NARTI > 0  { // natural ventilation
            X = TIDVL - c20
            if X > 0 {
                TIDVL = c20
                RRATE = DVENT * 1000/TIDVL
...
//            1420 PG=PG-BO2AD*C59
        // FOLLOWING SECTIONS NOT NEEDED FOR ME - these handle time displays in old FORTRAN MacPuf
//            1430 if(TND-60.) 1450,1440,1440 // counter in seconds
//            1440 TND=TND-60.
//            J3=J3+1         // time in minutes
//            1450 ND=INT(TND)  // time in seconds
