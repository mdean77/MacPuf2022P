...
extension Human {
    mutating func calculatePressures3(O2CT:Double, CO2CT:Double, pH:Double, temperature:Double = 37.0, DPG:Double = 3.7842, Hct:Double = 45.0,
    ...
    outerLoop: while ICH1 + ICH2 != 0 {
        ...
        (X, Y,  _ ) = calculateContents(pO2: RO2PR, pCO2: RC2PR, pH: RPH, temperature: TEMP, DPG: DPG, Hct: PCV/100, Hgb: HB)
    ...
    innerLoop: while  ICH3 + ICH4 < 1 {  // LINE 210
        ...
        DS=D2*Double(ICH2)
        X=RC2PR+DS-RC2PR*0.25
        if X < 0 {
            DS = -RC2PR * 0.75
        }
        RC2PR = RC2PR + DS
        
        if RO2PR < 0.1 { RO2PR = 0.1}
        if RC2PR < 0.1 { RC2PR = 0.1}
        ...
        if abs(XX2) < err {
            ...
            if XX2*D1 < 0 {
                ...
                ICH3=1
                RO2PR=XP1+(XP2-XP1)*abs(XX1)/(abs(XX2)+abs(XX1))
                D1Z=D1Z/2.0
            }
        }
        // SHOULD BE LINE 360
        ...
        if abs(YY2) < err {
            ...
            if YY2*D2 < 0 {
                ...
                ICH4=1
                RC2PR=YP1+(YP2-YP1)*abs(YY1)/(abs(YY2)+abs(YY1))
                D2Z=D2Z/2.0
            }
        }
        if ICH1 + ICH2 == 0 {
